...
// ProduceMessageWithSchema sends a message to a Kafka topic using Avro serialization
// with Schema Registry. The message is serialized in the Confluent wire format:
// [magic_byte(1)][schema_id(4)][avro_payload(n)]
func (c *producerClient) ProduceAvroMessage(ctx context.Context, key []byte, msg any) error {
	if !c.HasSchemaRegistry() {
		return fmt.Errorf("schema registry is not configured")
	}

	// Get the latest schema from Schema Registry
	latestSchema, err := c.schemaRegistry.GetLatestSchema(c.writer.Topic + "-value")
	if err != nil {
		return fmt.Errorf("failed to get latest schema for topic %s: %w", c.writer.Topic, err)
	}
	...
	if err := c.writer.WriteMessages(ctx, kafkago.Message{
		...
		return fmt.Errorf("failed to publish message to Kafka topic %s: %w", c.writer.Topic, err)
	}
